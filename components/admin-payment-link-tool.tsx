...
import { AdminNzdAudConverter } from "@/components/admin-nzd-aud-converter";
...
export function AdminPaymentLinkTool() {
  ...
  function applyAudFromConverter(aud: string) {
    if (mode === "invoice") {
      setInvoiceTotal(aud);
      return;
    }
    ...
    setUsePriceOverride(true);
    setCatalogOverride(aud);
  }
  ...
  return (
    <div className="ai-panel rounded-sm p-6 sm:p-8">
      ...
      <AdminNzdAudConverter
        className="mt-8"
        onApplyAud={applyAudFromConverter}
      />
